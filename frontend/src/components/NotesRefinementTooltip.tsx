...
interface NotesRefinementTooltipProps {
  notes: string;
  isVisible: boolean;
  onClose: () => void;
  onRefinedNotesSelect: (refinedNotes: string) => void;
}

const NotesRefinementTooltip: React.FC<NotesRefinementTooltipProps> = ({
  notes,
  isVisible,
  onClose,
  onRefinedNotesSelect,
}) => {
  const [isLoading, setIsLoading] = useState(false);
  const [refinedNotes, setRefinedNotes] = useState<{ [key: string]: string }>(
    {}
  );
  ...
  const handleRefinement = async (
    ...
    try {
      ...
      const result = await apiService.refineTaskNotes(notes, refinementType);

      setRefinedNotes((prev) => ({
        ...prev,
        [refinementType]: result.refinedNotes,
      }));
    } catch (error) {
      console.error("Failed to refine notes:", error);
      setError("Failed to refine notes. Please try again.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSelectRefinement = (refinementType: string) => {
    const refinedNote = refinedNotes[refinementType];
    if (refinedNote) {
      onRefinedNotesSelect(refinedNote);
      onClose();
    }
  };
...
export default NotesRefinementTooltip;
